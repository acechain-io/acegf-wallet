use acegf::{LegacyPrivKeyType, WeiToCryptoEntityUtil};
use acegf::{SignFlavor, ACEGF};
...
fn setup_signing() -> (String, &'static str) {
    let wallet = ACEGF::generate_internal("pass_for_bench", None).unwrap();
...
fn setup() -> (Uuid, String) {
    ...
    let wallet = ACEGF::generate_internal(PASS, None).unwrap();
...
fn benchmark_core_operations(c: &mut Criterion) {
    ...
    let xid_pub: String = unsafe {
        ...
        ACEGF::free_string(xid_pub_c_ptr);

        result
    };
    // ------------------------------------

    let mut group = c.benchmark_group("ACEGF_CORE_PERFORMANCE");
    group
        .sample_size(100)
    ...
    // =======================================================
    group.bench_function("A1. Derive_Four_Seed", |b| {
        b.iter(|| ACEGF::derive_from_uuid(black_box(&uuid)))
    });
    ...
    let _sealed_bytes = ACEGF::decode_mnemonic_to_sealed(&mn_sign).unwrap();

    group.bench_function("D1. Mnemonic_Decode_Sealed", |b| {
        b.iter(|| ACEGF::decode_mnemonic_to_sealed(black_box(MNEMONIC)))
    });
    ...
    group.bench_function("D2. Anchor_PK_to_UUID", |b| {
        b.iter(|| {
            WeiToCryptoEntityUtil::anchor_pk_to_uuid(LegacyPrivKeyType::Secp256k1, black_box(&pk32))
        })
    });
